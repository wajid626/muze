...
def create_spectrogram(args):
    mode = args.mode 
    if mode == "Train":
        if os.path.exists(os.getcwd() + '/torch/process_data/Train_Spectogram_Images'):
            return
        # Get Genres and Track IDs from the tracks.csv file
        filename_metadata =  os.getcwd() + "/torch/process_data/fma_metadata/tracks.csv"
        ...
        folder_sample =  os.getcwd() + "/torch/process_data/fma_small"
        ...
        if not os.path.exists(os.getcwd() + '/torch/process_data/Train_Spectogram_Images'):
            os.makedirs(os.getcwd() + '/torch/process_data/Train_Spectogram_Images')
            ...
            for f in file_names:
                ...
                if(str(tracks_genre_array[track_index, 0]) != '0'):
                    ...
                    try:
                      ...
                      plt.savefig(os.getcwd() + "/torch/process_data/Train_Spectogram_Images/"+str(counter)+"_"+str(tracks_genre_array[track_index,0])+".jpg", dpi=100)
                      plt.close()
                      counter = counter + 1
                    except:
        ...
        if os.path.exists(os.getcwd() + '/torch/process_data/Music_Spectogram_Images'):
            return
        folder_sample = os.getcwd() + '/torch/templates/music/'
       # folder_sample = "../templates/music/"
        counter = 0
        print("Converting mp3 audio files into mel Spectograms ...")
        if not os.path.exists(os.getcwd() + '/torch/process_data/Music_Spectogram_Images'):
            os.makedirs(os.getcwd() + '/torch/process_data/Music_Spectogram_Images')
        file_names = [os.path.join(folder_sample, f) for f in os.listdir(folder_sample)
                       if f.endswith(".mp3")]
        # Convert .mp3 files into mel-Spectograms
        for f in file_names:
            test_id = re.search(r'/Users/wajidabdul/courses/cis579/code/muze/torch/templates/music/(.+?).mp3', f).group(1)
            print(f)
            print(test_id)
            y, sr = librosa.load(f)
            melspectrogram_array = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=128,fmax=8000)
            ...
            plt.axes([0., 0., 1., 1.0], frameon=False, xticks=[], yticks=[])
            librosa.display.specshow(mel,cmap='gray_r')
            plt.savefig(os.getcwd() + '/torch/process_data/Music_Spectogram_Images/'+test_id+".jpg", dpi=100)
            plt.close()
        return

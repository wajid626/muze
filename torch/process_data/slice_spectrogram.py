...
def slice_spect(args):
    mode = args.mode 
    print(mode)
    train_sliced_spectogram_image_path =  './torch/process_data/Train_Sliced_Images'
    print(os.getcwd())
    if mode=="Train":
        if os.path.exists(train_sliced_spectogram_image_path):
            print('exists returning...')
            return
        labels = []
        image_folder = './torch/process_data/Train_Spectogram_Images'
        os.makedirs(train_sliced_spectogram_image_path)
        filenames = [os.path.join(image_folder, f) for f in os.listdir(image_folder)
                       if f.endswith(".jpg")]
        counter = 0
        print("Slicing Spectograms ...")
        if not os.path.exists(train_sliced_spectogram_image_path):
            os.makedirs(train_sliced_spectogram_image_path)
        for f in filenames:
            ...
            for i in range(round(number_of_samples)):
                ...
                img_temporary.save(train_sliced_spectogram_image_path + "/" +str(counter)+"_"+genre_variable+".jpg")
                counter += 1
        return

    elif mode=="Test":
        if os.path.exists(os.getcwd() + '/torch/process_data/Music_Sliced_Images'):
            return
        labels = []
        image_folder = os.getcwd() + '/torch/process_data/Music_Spectogram_Images'
        filenames = [os.path.join(image_folder, f) for f in os.listdir(image_folder)
                       if f.endswith(".jpg")]
        counter = 0
        print("Slicing Spectograms ...")
        if not os.path.exists(os.getcwd() + '/torch/process_data/Music_Sliced_Images'):
            os.makedirs(os.getcwd() + '/torch/process_data/Music_Sliced_Images')
        for f in filenames:
            ...
            for i in range(6, number_of_samples):
                ...
                img_temporary.save(os.getcwd() + '/torch/process_data/Music_Sliced_Images/' + str(counter)+ "_" + song_variable + ".jpg")
                counter = counter + 1
        return
